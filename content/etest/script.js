...
const app = {
    ...
    firebaseExams: [], // Store exams loaded from Firebase
    firebaseLoaded: false, // Flag to check if Firebase data is loaded
    ...
    async loadExamsFromFirebase() {
        try {
            if (window.firebaseEtest && window.firebaseEtest.getAllEtestExams) {
                const exams = await window.firebaseEtest.getAllEtestExams();
                this.firebaseExams = exams || [];
                this.firebaseLoaded = true;
                console.log('[E-test] Loaded', this.firebaseExams.length, 'exams from Firebase');
            }
        } catch (error) {
    ...
    showExamList(type) {
        // Try Firebase first, fallback to hard-coded
        let exams = [];
        if (this.firebaseLoaded && this.firebaseExams.length > 0) {
            exams = this.firebaseExams.filter(ex => ex.examType === type).map(ex => ({
                id: ex.id,
                title: ex.title,
                duration: `${Math.floor((ex.time || 3600) / 60)} phút`,
                count: (ex.sections || []).reduce((sum, s) => sum + (s.questions?.length || 0), 0)
            }));
        } else {
            exams = this.examLists[type] || [];
        }
    ...
    startExam(id) {
        // Try to find exam in Firebase data first, fallback to hard-coded
        let examData = null;
        if (this.firebaseLoaded && this.firebaseExams.length > 0) {
        ...
        this.data = examData || this.examContent[id] || this.examContent['thpt_2024'];
